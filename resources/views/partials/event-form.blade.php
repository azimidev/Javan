...
<div class="form-group">
	...
	<div class="col-sm-10">
		<input type="number" class="form-control" name="capacity" id="capacity" placeholder="Capacity"
		       value="{{ old('capacity') ?? $events->capacity }}" min="1" max="80" required>
...
<div class="form-group">
	...
	<div class="col-sm-10">
		<input class="form-control" type="text" name="start" id="start" placeholder="YYY-MM-DD HH:MM:SS"
		       value="{{ old('start') ?? $events->start }}" pattern="[0-9:\-\s]+" required>
		<span class="help-block text-primary">Event start date format {{ date('Y-m-d H:i:s') }}</span>
	</div>
</div>
...
<div class="form-group">
	...
	<div class="col-sm-10">
		<input class="form-control" type="text" name="finish" id="finish" placeholder="YYY-MM-DD HH:MM:SS"
		       value="{{ old('finish') ?? $events->finish }}" pattern="[0-9:\-\s]+" required>
		<span class="help-block text-primary">Event finish date format {{ date('Y-m-d H:i:s') }}</span>
	</div>
</div>
